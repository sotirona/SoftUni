...
namespace _10.TheHeiganDance
{
    class TheHeiganDance
    {
        ...
        private static bool IsInsideMatrix(int[] escapePosition)
        {
            int size = 15;
            for (int i = 0; i < 2; i++)
                if (escapePosition[i] < 0 || escapePosition[i] >= size) return false;
            return true;
        }

        private static bool IsHitBySpell(int[] spellCoordinates, int[] playerPosition)
        {
